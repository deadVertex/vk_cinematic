...
layout(binding = 11) uniform texture2D computeShaderImage;
...
void main()
{
    ...
    else if (ubo.showComparision == 3)
    {
        // Compute shader image
        color = texture(sampler2D(computeShaderImage, defaultSampler), fragTexCoord).rgb;
    }
